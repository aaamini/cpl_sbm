...
TAG = '_large_gam00_w111_Ea';
figName = {};
figName{1} = ['time_n' TAG];
%figName{2} = ['pri' TAG];
%figName{3} = ['P' TAG];
dataFile = ['time' TAG '.mat'];
...
lambda = 7;
nV = floor(logspace(log10(500),log10(1e6),15));   % "V"ector of "O"ut-"I"n-"R"atio
%nV = [499, 997, 1991, 3974, 7931, 15828, 31588, 63041, ...
%    125811, 251080, 501079, 1000000];
%nV = nV(end-1);
%threshV = zeros(size(nV));
...
%%
[GdTV Gerr Gerr_pri Gerr_P] = deal( zeros(numMtds, numel(nV)) );
...
for r = 1:numel(nV)
    ...
    fprintf(1,'\n n = %5d, low = (%2.2f, %2.2f), w = (%s) [%3d of %3d  ]\n', ...
        n, lowVal, lowProb, num2str(inWei), r, numel(nV)) 
    ...
    [dTV err err_pri err_P] = deal( zeros(numMtds, numel(nV), Nrea) );       
...
%%
textStr = sprintf(...
          ['\\lambda = %1.2f,  \\gamma = %1.1f,  \\beta = %1.2f,  ' ...
          'K = %2d,  N_{rea} = %d\n'], ...
          lambda, lowProb, oir, K, Nrea);
...
GGERR = {log10(GdTV), log10(Gerr_pri), log10(Gerr_P)};
yLIMs = {[-2 5],[-1.75 0.5],[-0.5 1.5]};
% legLOC = {'SouthEast','NorthEast','SouthEast'};
% txtLOC = {[0.1,0.96],[0.3,0.96],[0.3,0.16]};
legLOC = {'NorthWest','NorthEast','NorthEast'};
txtLOC = {[0.35,0.16],[0.3,0.96],[0.3,0.96]};
...
for FIG = 1:nFig
    ...
    for id = 1:numIDX
        k = IDX(id);
        ph(id) = plot(log10(nV), GGERR{FIG}(k,:), style{id}, 'color', color{id});
    end

    set(ph,'LineWidth',2)
    hYLabel = ylabel(yLABs{FIG});
    hXLabel = xlabel('log_{10} n');
    set([hXLabel, hYLabel], 'FontName', 'Arial', 'FontSize',14,'FontWeight','bold')

    axis([[min(log10(nV)), max(log10(nV))] yLIMs{FIG}])

    th = text(txtLOC{FIG}(1),txtLOC{FIG}(2),textStr,'FontSize',14,'FontWeight','bold','units','normalized');
